...
{
	img_sub = node.subscribe("/ardrone/image_raw", 1, &CircleTest::imageCallback, this);//"/ardrone/image_raw" "CamImage"
	altitude_sub = node.subscribe("/ardrone/navdata_altitude", 1, &CircleTest::altitudeCallback, this);
	navdata_sub = node.subscribe("/ardrone/navdata", 1, &CircleTest::navdataCallback, this);
...
void CircleTest::altitudeCallback(const ardrone_autonomy::navdata_altitude &msg)
{
...
void CircleTest::navdataCallback(const ardrone_autonomy::Navdata &msg)
{
...
void CircleTest::imageCallback(const sensor_msgs::Image &msg)
{
...
void CircleTest::image_process(Mat img)
{
...
void CircleTest::find_blue(Mat &img, Mat imgThresholded)
{
...
void CircleTest::find_yellow(Mat &img, Mat imgThresholded)
{
	for (int i = 0; i < img.rows; ++i)
	{
		for (int j = 0; j < img.cols; ++j)
...
void CircleTest::find_red(Mat &img, Mat imgThresholded)
{
	for (int i = 0; i < img.rows; ++i)
	{
		for (int j = 0; j < img.cols; ++j)
...
/*void CircleTest::line_fitting(ardrone_trajectory::points reds, double &k, double &b, double &r, geometry_msgs::Point &foot)
{
	double xmean = 0;
	double ymean = 0;
	double x2mean = 0;
...
void CircleTest::measurement(double &x, double &y, double u, double v, double pitch, double roll, double h)
{
...
void CircleTest::revmeasurement(double x, double y, double &u, double &v, double pitch, double roll, double h)
{
